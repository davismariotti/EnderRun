...
public class EnderChunkGenerator extends ChunkGenerator {
	...
	/**
	 * 
	 * @param IslandSize The size (on average) of each island
	 * @param IslandDistance The average distance between islands
	 * @param chunkRadiusGenerated The number of chunks outward
	 * the generator will generate from (0,0)
	 */
	public EnderChunkGenerator(int IslandSizeX, int islandSizeZ, int IslandDistance, int chunkRadiusGenerated) {
	...
	public static void createNewEnderRunWorld() {
		...
		//TODO Add some code to generate the entire world, not just spawn
	...
	public byte[][] generateBlockSections(World world, Random rand, int chunkX,
		...
		if (chunkX > islandSizeX
				|| chunkX < -islandSizeX
				|| chunkZ > islandSizeZ
				|| chunkZ > -islandSizeZ) return null;
		
		PerlinOctaveGenerator blockDensityGenerator = new PerlinOctaveGenerator(world,8);
		...
		for (int x=0; x<16; x++) {
			for (int z=0; z<16; z++) {
				...
				for (int y=64;y<128;y++) {					
					double density = blockDensityGenerator.noise(worldX,y, worldZ, 1, 1);					
	...
	/**
	 * Returns a list of all of the block populators (they make "little" features)
	 * to be called after the chunk generator runs on this chunk
	 */
	@Override
